...
class BitfinexTickerMapper:
    """
    Maps Bitfinex WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'bitfinex') -> Optional[int]:
        """
        Get vendor ID for Bitfinex.

        Args:
            vendor_name: Vendor name (default: 'bitfinex')
    ...
    def create_field_mapping(
        ...
        Args:
            ...
            vendor_field_path: Vendor-specific field path (e.g., 'BID' for Bitfinex bid price)
    ...
    def map_bitfinex_ticker_fields(self):
        """
        Map Bitfinex WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting Bitfinex ticker field mapping...")
        ...
        if not channel_id:
            logger.error("Bitfinex ticker channel not found")
            return 0

        # Define mappings for Bitfinex ticker
        # Bitfinex field names are already descriptive
        # Format: (vendor_field_path, canonical_field_name, transformation_rule, entity_type)
        mappings = [
            # Core price fields - Bitfinex uses float strings
            ('BID', 'bid_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('BID_SIZE', 'bid_size', {'type': 'string_to_numeric'}, 'ticker'),
            ('ASK', 'ask_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('ASK_SIZE', 'ask_size', {'type': 'string_to_numeric'}, 'ticker'),
            ('LAST_PRICE', 'last_price', {'type': 'string_to_numeric'}, 'ticker'),
            ...
            # Symbol field - Bitfinex doesn't have symbol in ticker message?
            # Actually Bitfinex ticker doesn't include symbol in the message schema shown,
            # but channel is subscribed to specific symbol
            # We'll skip symbol mapping for now

            # Channel ID as sequence
            ('CHANNEL_ID', 'sequence', {'type': 'string_to_integer'}, 'ticker'),

            # Daily change fields - no direct canonical equivalents
            # ('DAILY_CHANGE', 'daily_change', {'type': 'string_to_numeric'}, 'ticker'),
            # ('DAILY_CHANGE_RELATIVE', 'daily_change_pct', {'type': 'string_to_numeric'}, 'ticker'),

            # Note: Bitfinex doesn't have open_24h in ticker schema
            # Note: Bitfinex doesn't have volume_30d in ticker schema
        ]
        ...
        logger.info(f"Bitfinex ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        # Count total mappings for Bitfinex ticker
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'bitfinex' AND data_type_name = 'ticker'
        """)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Map Bitfinex WebSocket ticker fields to canonical fields",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create Bitfinex ticker mappings
  python create_bitfinex_mappings.py

  # Create mappings and verify results
  python create_bitfinex_mappings.py --verify

  # Dry run (show what would be mapped without actually creating)
  python create_bitfinex_mappings.py --dry-run
        """
    )
    ...
    with BitfinexTickerMapper() as mapper:
        if args.dry_run:
            print("Dry run mode - would map the following fields:")
            print("=" * 60)

            # Get vendor and channel IDs for display
            vendor_id = mapper.get_vendor_id('bitfinex')
            channel_id = mapper.get_websocket_channel_id(vendor_id, 'ticker')

            print(f"Vendor: bitfinex (id={vendor_id})")
            print(f"Channel: ticker (id={channel_id})")
            print()

            # Show what would be mapped
            mappings = [
                ('BID', 'bid_price', 'ticker', 'float'),
                ('BID_SIZE', 'bid_size', 'ticker', 'float'),
                ('ASK', 'ask_price', 'ticker', 'float'),
                ('ASK_SIZE', 'ask_size', 'ticker', 'float'),
                ('LAST_PRICE', 'last_price', 'ticker', 'float'),
                ('HIGH', 'high_24h', 'ticker', 'float'),
                ('LOW', 'low_24h', 'ticker', 'float'),
                ('VOLUME', 'volume_24h', 'ticker', 'float'),
                ('CHANNEL_ID', 'sequence', 'ticker', 'integer'),
            ]

            for vendor_path, canonical_name, entity_type, data_type in mappings:
                field_id = mapper.get_canonical_field_id(canonical_name)
                status = "✓" if field_id else "✗ (field not found)"
                print(f"{vendor_path:15} → {canonical_name:20} [{entity_type:10}] {data_type:10} {status}")
            ...
            # Create mappings
            created = mapper.map_bitfinex_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} Bitfinex ticker field mappings")
