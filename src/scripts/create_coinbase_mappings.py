...
class CoinbaseTickerMapper:
    """
    Maps Coinbase WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'coinbase') -> Optional[int]:
        """
    ...
    def map_coinbase_ticker_fields(self):
        """
        Map Coinbase WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting Coinbase ticker field mapping...")
        ...
        if not channel_id:
            logger.error("Coinbase ticker channel not found")
            return 0
        ...
        mappings = [
            ...
            ('ticker.best_bid', 'bid_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('ticker.best_ask', 'ask_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('ticker.price', 'last_price', {'type': 'string_to_numeric'}, 'ticker'),

            # Volume fields (string to numeric conversion)
            ('ticker.volume_24h', 'volume_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('ticker.volume_30d', 'volume_30d', {'type': 'string_to_numeric'}, 'ticker'),

            # High/Low/Open fields (string to numeric conversion)
            ('ticker.high_24h', 'high_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('ticker.low_24h', 'low_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('ticker.open_24h', 'open_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # Symbol/ID fields
            ('ticker.product_id', 'symbol', {'type': 'identity'}, 'ticker'),

            # Timestamp field (string to datetime conversion)
            ('ticker.time', 'timestamp', {'type': 'string_to_datetime', 'format': 'iso8601'}, 'ticker'),

            # Sequence field (string to integer conversion)
            ('ticker.sequence', 'sequence', {'type': 'string_to_integer'}, 'ticker'),

            # Trade-related fields (map to trade entity type for completeness)
            ('ticker.trade_id', 'trade_id', {'type': 'identity'}, 'trade'),
            ('ticker.side', 'side', {'type': 'identity'}, 'trade'),

            # Note: ticker.last_size could map to size in trade context, but it's not in ticker schema
            # ('ticker.last_size', 'size', {'type': 'string_to_numeric'}, 'trade'),
        ]
        ...
        logger.info(f"Coinbase ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'coinbase' AND data_type_name = 'ticker'
        """)
...
Examples:
  ...
  python create_coinbase_mappings.py --dry-run
    ...
    with CoinbaseTickerMapper() as mapper:
        if args.dry_run:
            print("Dry run mode - would map the following fields:")
            print("=" * 60)

            # Get vendor and channel IDs for display
            vendor_id = mapper.get_vendor_id('coinbase')
            channel_id = mapper.get_websocket_channel_id(vendor_id, 'ticker')

            print(f"Vendor: coinbase (id={vendor_id})")
            print(f"Channel: ticker (id={channel_id})")
            print()

            # Show what would be mapped
            mappings = [
                ('ticker.best_bid', 'bid_price', 'ticker'),
                ('ticker.best_ask', 'ask_price', 'ticker'),
                ('ticker.price', 'last_price', 'ticker'),
                ('ticker.volume_24h', 'volume_24h', 'ticker'),
                ('ticker.volume_30d', 'volume_30d', 'ticker'),
                ('ticker.high_24h', 'high_24h', 'ticker'),
                ('ticker.low_24h', 'low_24h', 'ticker'),
                ('ticker.open_24h', 'open_24h', 'ticker'),
                ('ticker.product_id', 'symbol', 'ticker'),
                ('ticker.time', 'timestamp', 'ticker'),
                ('ticker.sequence', 'sequence', 'ticker'),
                ('ticker.trade_id', 'trade_id', 'trade'),
                ('ticker.side', 'side', 'trade'),
            ]
            ...
            # Create mappings
            created = mapper.map_coinbase_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} Coinbase ticker field mappings")
