#!/usr/bin/env python3
"""
Create KuCoin WebSocket ticker field mappings to canonical fields.
Maps KuCoin-specific field names to industry-standard canonical field names.
"""
...
class KucoinTickerMapper:
    """
    Maps KuCoin WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'kucoin') -> Optional[int]:
        """
        Get vendor ID for KuCoin.

        Args:
            vendor_name: Vendor name (default: 'kucoin')
    ...
    def map_kucoin_ticker_fields(self):
        """
        Map KuCoin WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting KuCoin ticker field mapping...")

        # Get vendor and channel IDs
        vendor_id = self.get_vendor_id('kucoin')
        if not vendor_id:
        ...
        if not channel_id:
            logger.error("KuCoin ticker channel not found")
            return 0
        ...
        mappings = [
            ...
            ('data.price', 'last_price', {'type': 'string_to_numeric'}, 'ticker'),

            # Size fields (string to numeric conversion)
            ('data.bestBidSize', 'best_bid_size', {'type': 'string_to_numeric'}, 'ticker'),
            ('data.bestAskSize', 'best_ask_size', {'type': 'string_to_numeric'}, 'ticker'),
            ('data.size', 'size', {'type': 'string_to_numeric'}, 'trade'),

            # Sequence field (string to integer conversion)
            ('data.sequence', 'sequence', {'type': 'string_to_integer'}, 'ticker'),

            # Timestamp field (nanoseconds to datetime conversion)
            ('data.time', 'timestamp', {'type': 'ms_to_datetime'}, 'ticker'),
            # Note: KuCoin time is in nanoseconds, ms_to_datetime will handle if we adjust factor
            # Actually ms_to_datetime expects milliseconds, so we need custom transformation
            # We'll use ms_to_datetime and handle conversion in normalization engine
        ]
        ...
        logger.info(f"KuCoin ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'kucoin' AND data_type_name = 'ticker'
        """)
    ...
    with KucoinTickerMapper() as mapper:
        if args.dry_run:
            ...
            mappings = [
                ...
                ('data.price', 'last_price', 'ticker'),
                ('data.bestBidSize', 'best_bid_size', 'ticker'),
                ('data.bestAskSize', 'best_ask_size', 'ticker'),
                ('data.size', 'size', 'trade'),
                ('data.sequence', 'sequence', 'ticker'),
                ('data.time', 'timestamp', 'ticker'),
            ]
            ...
            # Create mappings
            created = mapper.map_kucoin_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} KuCoin ticker field mappings")
