#!/usr/bin/env python3
"""
Create Gate.io WebSocket ticker field mappings to canonical fields.
Maps Gate.io-specific field names to industry-standard canonical field names.
"""
...
class GateioTickerMapper:
    """
    Maps Gate.io WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'gateio') -> Optional[int]:
        """
        Get vendor ID for Gate.io.

        Args:
            vendor_name: Vendor name (default: 'gateio')
    ...
    def map_gateio_ticker_fields(self):
        """
        Map Gate.io WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting Gate.io ticker field mapping...")

        # Get vendor and channel IDs
        vendor_id = self.get_vendor_id('gateio')
        if not vendor_id:
            logger.error("Gate.io vendor not found")
            return 0

        channel_id = self.get_websocket_channel_id(vendor_id, 'tickers')
        if not channel_id:
            logger.error("Gate.io tickers channel not found")
            return 0
        ...
        mappings = [
            ...
            ('result.lowest_ask', 'ask_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('result.highest_bid', 'bid_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('result.last', 'last_price', {'type': 'string_to_numeric'}, 'ticker'),

            # Volume fields (string to numeric conversion)
            ('result.base_volume', 'volume_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # High/Low fields (string to numeric conversion)
            ('result.high_24h', 'high_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('result.low_24h', 'low_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # Symbol/ID fields
            ('result.currency_pair', 'symbol', {'type': 'identity'}, 'ticker'),

            # Timestamp field (milliseconds to datetime conversion)
            ('time_ms', 'timestamp', {'type': 'ms_to_datetime'}, 'ticker'),
        ]
        ...
        logger.info(f"Gate.io ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'gateio' AND data_type_name = 'ticker'
        """)
...
Examples:
  ...
  python create_coinbase_mappings.py --dry-run
    ...
    with GateioTickerMapper() as mapper:
        if args.dry_run:
            print("Dry run mode - would map the following fields:")
            print("=" * 60)

            # Get vendor and channel IDs for display
            vendor_id = mapper.get_vendor_id('gateio')
            channel_id = mapper.get_websocket_channel_id(vendor_id, 'tickers')

            print(f"Vendor: gateio (id={vendor_id})")
            print(f"Channel: tickers (id={channel_id})")
            print()

            # Show what would be mapped
            mappings = [
                ('result.lowest_ask', 'ask_price', 'ticker'),
                ('result.highest_bid', 'bid_price', 'ticker'),
                ('result.last', 'last_price', 'ticker'),
                ('result.base_volume', 'volume_24h', 'ticker'),
                ('result.high_24h', 'high_24h', 'ticker'),
                ('result.low_24h', 'low_24h', 'ticker'),
                ('result.currency_pair', 'symbol', 'ticker'),
                ('time_ms', 'timestamp', 'ticker'),
            ]
            ...
            # Create mappings
            created = mapper.map_gateio_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} Gate.io ticker field mappings")
