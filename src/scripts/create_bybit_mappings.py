...
class BybitTickerMapper:
    """
    Maps Bybit WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'bybit') -> Optional[int]:
        """
        Get vendor ID for Bybit.

        Args:
            vendor_name: Vendor name (default: 'bybit')
    ...
    def get_websocket_channel_id(self, vendor_id: int, channel_name: str = 'tickers') -> Optional[int]:
        """
        Get WebSocket channel ID for tickers channel.
    ...
    def map_bybit_ticker_fields(self):
        """
        Map Bybit WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting Bybit ticker field mapping...")

        # Get vendor and channel IDs
        vendor_id = self.get_vendor_id('bybit')
        if not vendor_id:
            logger.error("Bybit vendor not found")
            return 0

        channel_id = self.get_websocket_channel_id(vendor_id, 'tickers')
        if not channel_id:
            logger.error("Bybit tickers channel not found")
            return 0
        ...
        mappings = [
            ...
            ('data.lastPrice', 'last_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('data.highPrice24h', 'high_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('data.lowPrice24h', 'low_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # Volume fields (string to numeric conversion)
            ('data.volume24h', 'volume_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # Symbol/ID fields
            ('data.symbol', 'symbol', {'type': 'identity'}, 'ticker'),

            # Timestamp field (milliseconds to datetime conversion)
            ('ts', 'timestamp', {'type': 'ms_to_datetime'}, 'ticker'),
        ]
        ...
        logger.info(f"Bybit ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'bybit' AND data_type_name = 'ticker'
        """)
...
Examples:
  ...
  python create_coinbase_mappings.py --dry-run
    ...
    with BybitTickerMapper() as mapper:
        if args.dry_run:
            print("Dry run mode - would map the following fields:")
            print("=" * 60)

            # Get vendor and channel IDs for display
            vendor_id = mapper.get_vendor_id('bybit')
            channel_id = mapper.get_websocket_channel_id(vendor_id, 'tickers')

            print(f"Vendor: bybit (id={vendor_id})")
            print(f"Channel: tickers (id={channel_id})")
            print()

            # Show what would be mapped
            mappings = [
                ('data.lastPrice', 'last_price', 'ticker'),
                ('data.highPrice24h', 'high_24h', 'ticker'),
                ('data.lowPrice24h', 'low_24h', 'ticker'),
                ('data.volume24h', 'volume_24h', 'ticker'),
            ...
            # Create mappings
            created = mapper.map_bybit_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} Bybit ticker field mappings")
