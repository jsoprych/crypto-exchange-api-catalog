#!/usr/bin/env python3
"""
Create OKX WebSocket ticker field mappings to canonical fields.
Maps OKX-specific field names to industry-standard canonical field names.
"""
...
class OkxTickerMapper:
    """
    Maps OKX WebSocket ticker fields to canonical fields.
    """
    ...
    def get_vendor_id(self, vendor_name: str = 'okx') -> Optional[int]:
        """
        Get vendor ID for Coinbase.

        Args:
            vendor_name: Vendor name (default: 'coinbase')
    ...
    def get_websocket_channel_id(self, vendor_id: int, channel_name: str = 'tickers') -> Optional[int]:
        """
    ...
    def create_field_mapping(
        ...
        Args:
            ...
            vendor_field_path: Vendor-specific field path (e.g., 'ticker.best_bid')
    ...
    def map_okx_ticker_fields(self):
        """
        Map OKX WebSocket ticker fields to canonical fields.

        Returns:
            Number of mappings created
        """
        logger.info("Starting OKX ticker field mapping...")

        # Get vendor and channel IDs
        vendor_id = self.get_vendor_id('okx')
        if not vendor_id:
            logger.error("OKX vendor not found")
            return 0

        channel_id = self.get_websocket_channel_id(vendor_id, 'tickers')
        if not channel_id:
            logger.error("OKX tickers channel not found")
            return 0
        ...
        mappings = [
            ...
            ('data[0].bidPx', 'bid_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('data[0].askPx', 'ask_price', {'type': 'string_to_numeric'}, 'ticker'),
            ('data[0].last', 'last_price', {'type': 'string_to_numeric'}, 'ticker'),

            # Size fields (string to numeric conversion)
            ('data[0].bidSz', 'best_bid_size', {'type': 'string_to_numeric'}, 'ticker'),
            ('data[0].askSz', 'best_ask_size', {'type': 'string_to_numeric'}, 'ticker'),
            ('data[0].lastSz', 'last_size', {'type': 'string_to_numeric'}, 'trade'),

            # Volume fields (string to numeric conversion)
            ('data[0].vol24h', 'volume_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # High/Low/Open fields (string to numeric conversion)
            ('data[0].high24h', 'high_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('data[0].low24h', 'low_24h', {'type': 'string_to_numeric'}, 'ticker'),
            ('data[0].open24h', 'open_24h', {'type': 'string_to_numeric'}, 'ticker'),

            # Symbol/ID fields
            ('data[0].instId', 'symbol', {'type': 'identity'}, 'ticker'),

            # Timestamp field (milliseconds to datetime conversion)
            ('data[0].ts', 'timestamp', {'type': 'ms_to_datetime'}, 'ticker'),

            # Note: OKX doesn't have sequence number in ticker
            # Note: OKX doesn't have trade_id or side in ticker (those are in trade channel)
        ]
        ...
        logger.info(f"OKX ticker mapping complete: {created_count} created, {failed_count} failed")
    ...
    def verify_mappings(self) -> Dict[str, int]:
        ...
        # Count total mappings for Coinbase ticker
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            WHERE vendor_name = 'okx' AND data_type_name = 'ticker'
        """)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Map Coinbase WebSocket ticker fields to canonical fields",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create Coinbase ticker mappings
  python create_coinbase_mappings.py

  # Create mappings and verify results
  python create_coinbase_mappings.py --verify

  # Dry run (show what would be mapped without actually creating)
  python create_coinbase_mappings.py --dry-run
        """
    )
    ...
    with OkxTickerMapper() as mapper:
        if args.dry_run:
            print("Dry run mode - would map the following fields:")
            print("=" * 60)

            # Get vendor and channel IDs for display
            vendor_id = mapper.get_vendor_id('okx')
            channel_id = mapper.get_websocket_channel_id(vendor_id, 'tickers')

            print(f"Vendor: okx (id={vendor_id})")
            print(f"Channel: tickers (id={channel_id})")
            print()

            # Show what would be mapped
            mappings = [
                ('data[0].bidPx', 'bid_price', 'ticker'),
                ('data[0].askPx', 'ask_price', 'ticker'),
                ('data[0].last', 'last_price', 'ticker'),
                ('data[0].bidSz', 'best_bid_size', 'ticker'),
                ('data[0].askSz', 'best_ask_size', 'ticker'),
                ('data[0].lastSz', 'size', 'trade'),
                ('data[0].vol24h', 'volume_24h', 'ticker'),

                ('data[0].high24h', 'high_24h', 'ticker'),
                ('data[0].low24h', 'low_24h', 'ticker'),
                ('data[0].open24h', 'open_24h', 'ticker'),
                ('data[0].instId', 'symbol', 'ticker'),
                ('data[0].ts', 'timestamp', 'ticker'),
            ]
            ...
            # Create mappings
            created = mapper.map_okx_ticker_fields()

            if created > 0:
                print(f"\n✓ Created {created} OKX ticker field mappings")
