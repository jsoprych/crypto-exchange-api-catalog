...
class HuobiAdapter(BaseVendorAdapter):
    """
    Template adapter for Huobi Exchange API.

    Replace all occurrences of:
    - 'TemplateAdapter' with '[ExchangeName]Adapter' (e.g., 'BybitAdapter')
    - 'Huobi' with actual exchange name (e.g., 'Bybit')
    - 'https://api.huobi.pro' with actual REST API base URL
    - 'wss://api.huobi.pro/ws' with actual WebSocket URL
    ...
    def discover_rest_endpoints(self) -> List[Dict[str, Any]]:
        """
        Discover Huobi REST API endpoints.
        ...
        """
        logger.info("Discovering Huobi REST endpoints")
    ...
    def discover_websocket_channels(self) -> List[Dict[str, Any]]:
        """
        Discover Huobi WebSocket channels and message formats.
        ...
        """
        logger.info("Discovering Huobi WebSocket channels")
    ...
    def discover_products(self) -> List[Dict[str, Any]]:
        """
        Discover Huobi trading products/symbols from live API.

        Uses Huobi API: /v1/common/symbols
        Documentation: https://huobiapi.github.io/docs/spot/v1/en/#get-all-supported-trading-symbol

        Returns:
            List of product dictionaries in standard format
        """
        logger.info("Discovering Huobi products from live API")

        try:
            # ========================================================================
            # 1. FETCH PRODUCTS FROM HUOBI API
            # ========================================================================

            # Huobi symbols endpoint for all trading pairs
            products_url = f"{self.base_url}/v1/common/symbols"

            logger.debug(f"Fetching Huobi products from: {products_url}")

            # Make the API request
            response = self.http_client.get(products_url)

            # ========================================================================
            # 2. PARSE HUOBI RESPONSE FORMAT
            # ========================================================================

            # Huobi response format: {"status": "ok", "data": [...]}
            if response.get("status") != "ok":
                error_msg = response.get("err-msg", "Unknown error")
                logger.error(f"Huobi API error: {error_msg}")
                raise Exception(f"Huobi API error: {error_msg}")
            ...
            if not isinstance(symbols_data, list):
                ...
                raise Exception(f"Unexpected response format from Huobi")
            ...
            for symbol_info in symbols_data:
                try:
                    # Extract symbol information from Huobi response
                    symbol = symbol_info.get("symbol", "")  # e.g., "btcusdt"
                    base_currency = symbol_info.get("base-currency", "")
                    quote_currency = symbol_info.get("quote-currency", "")

                    # Status mapping for Huobi
                    state = symbol_info.get("state", "")
                    if state == "online":
                        status = "online"
                    elif state in ["offline", "suspend"]:
                        status = "offline"
                    elif state == "delisted":
                        status = "delisted"
                    else:
                    ...
                    # Trading limits and precision from Huobi response
                    min_order_size = None
                    max_order_size = None
                    price_increment = None

                    # Minimum order size (amount precision)
                    min_order_amt = symbol_info.get("min-order-amt")
                    if min_order_amt:
                        min_order_size = float(min_order_amt)

                    # Maximum order size (max-order-amt)
                    max_order_amt = symbol_info.get("max-order-amt")
                    if max_order_amt:
                        max_order_size = float(max_order_amt)

                    # Price increment (price precision)
                    price_precision = symbol_info.get("price-precision")
                    if price_precision is not None:
                        try:
                            price_increment = 10 ** -int(price_precision)
                        except (ValueError, TypeError):
                            pass

                    # Additional precision information
                    amount_precision = symbol_info.get("amount-precision")
                    value_precision = symbol_info.get("value-precision")
                    limit_order_min_order_amt = symbol_info.get("limit-order-min-order-amt")
                    limit_order_max_order_amt = symbol_info.get("limit-order-max-order-amt")
                    sell_market_min_order_amt = symbol_info.get("sell-market-min-order-amt")
                    sell_market_max_order_amt = symbol_info.get("sell-market-max-order-amt")
                    buy_market_max_order_value = symbol_info.get("buy-market-max-order-value")
                    ...
                    product = {
                        ...
                        "vendor_metadata": {
                            ...
                            "symbol_partition": symbol_info.get("symbol-partition"),
                            "state": state,
                            ...
                            "limit_order_min_order_amt": limit_order_min_order_amt,
                            "limit_order_max_order_amt": limit_order_max_order_amt,
                            "sell_market_min_order_amt": sell_market_min_order_amt,
                            "sell_market_max_order_amt": sell_market_max_order_amt,
                            "buy_market_max_order_value": buy_market_max_order_value,
                            "leverage_ratio": symbol_info.get("leverage-ratio"),
                            "api_trading": symbol_info.get("api-trading")
                        }
                    }
                    ...
                    logger.warning(f"Failed to parse Huobi product {symbol_info.get('symbol', 'unknown')}: {e}")
                    continue
            ...
            if not products:
                logger.error("No products discovered from Huobi API response")
                raise Exception("No products found in Huobi API response")
            ...
            logger.error(f"Failed to discover Huobi products: {e}")
            # Re-raise to ensure discovery run is marked as failed
            raise Exception(f"Product discovery failed for Huobi: {e}")
            raise Exception(f"Product discovery failed for Huobi: {e}")
